...
function existsOrCreate(filePath, opts, cb){
    ...
    fs.exists(fp.fullPath, function(exists){
        if(exists && !opts.replace) cb();
            ...
            // file doesnt exists, create folder first
 ...
 */
function watchRecursive(dir, opts, onChange, cb){ // cb(err, files)
    ...
    dir = walk(dir, opts, function(err, items){
        if(err) cb(new Error('fsExt.watchRecursive: exploring folder failed').cause(err));
        else {
        ...
        for(var key in items){
            if(items[key].isDir && !_dirWatchers[ items[key].fullPath ]) { // avoid duplicite watchers
                ...
                try {
                    ...
                    _dirWatchers[ item.fullPath ] = fs.watch(item.fullPath).on('change', function(op, fileName){
