...
const __record__ = function(start, end, type) {
    console.log(`Start byte: ${start}, end byte: ${end}, type: ${type}`);
}
        ...
        exit: function(path, state) {
            if (path.isMemberExpression()) {
                ...
                let funcCallExpr = babelTypes.callExpression(
                    ...
                    [babelParser.parseExpression(__record__.toString())]
                );
                
                
                path.node.body = [funcCallExpr];
                path.node.directives = [];
                path.skip();
