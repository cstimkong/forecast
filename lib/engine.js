 ...
 */
function preAnalysis(func, thisArg, globalContext) {
...
export function mainProcess(modulePath, opts) {
    ...
    let [lib, sourceFiles] = loadNodeJSModule(modulePath, {returnSourceFiles: true});
    ...
    for (let [f, functionName, _this] of funcs) {
        let [templateArgArrays, runtimeHints] = preAnalysis(f, _this, globalContext);
    ...
    for (let [_, funcname, _this] of funcs) {
        ...
        for (let templateArgArray of totalTemplateArgArrays[funcname]) {
            ...
            if (accepted) {
                generateExploit(templateArgArray);
            }
        }
    }
}
