 ...
 */
function generateTemplateArgArrays(func, thisArg) {
    let skeletenArrayHashMap = {};
    for (let i = 0; i < MAX_FORCED_EXECUTION_COUNT; i++) {
        try {
            let argArray = forcedExecution(func, Math.floor(Math.random() * 6), thisArg);
            let skeletonArray = [];
    ...
    return Object.values(templateArgArrayHashMap);
}
...
function __getcreationlocation__(obj) {
    if (Object.hasOwn(obj, '__creationlocation__')) {
        return obj.__creationlocation__;
    }

    return undefined;
}

module.exports = { generateTemplateArgArrays };